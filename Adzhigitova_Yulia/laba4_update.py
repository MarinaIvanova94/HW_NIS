...
for i in range(0, len(url_list)):
    news = SubElement(root, 'news')
    SubElement(news, 'title', href=url_list[i], title = news_title_list[i])
    SubElement(news, 'summary').text = an_news_list[i]
...
f1 = open('D:\NIS\HW_NIS\Adzhigitova_Yulia\laba4.xml', 'wb')
f1.write(output)
print(output)
f1.close()
